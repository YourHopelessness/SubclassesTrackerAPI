...
namespace SubclassesTracker.Api.EsologsServices.Reports
{
    public partial class ReportSubclassesDataService
    {
            ...
            bool filterNeeded = true)
        {
            ...
            foreach (var (Key, PlayerId, LogId, FightId, FightScore, Talents, BaseClass) in bestCandidates)
            {
                var key = (Key.PlayerName, Key.Role);

                if (seenNameRolePairs.Contains(key)
                    && !Key.PlayerName.Equals("nil", StringComparison.InvariantCultureIgnoreCase))
                    continue;
